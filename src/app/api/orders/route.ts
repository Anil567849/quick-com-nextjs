...
export async function POST(request: Request) {
    ...
    let transactionError: string = ''; // for storing error
    let finalOrder: any = null;


    // Creating a transaction  - Either all query pass or fail.
    try {
        finalOrder = await db.transaction(async (tx) => { // Creating transaction - set of multiple query
            // create order
            const order = await tx
                ...
                .returning({ id: orders.id, price: orders.price }); // we need id and price after inserting
            ...
            const availableStock = await tx
                ...
                .where(
                    and( // all condition should be met
                        ...
                        isNull(inventories.orderId) // is order null in inventories
                    )
                )
                .limit(validatedData.qty) // we need row == qty
                .for('update', { skipLocked: true }); // lock those row from update [skipLocked = if new user's order place when old user's order is on process then skip those row for new user]

            if (availableStock.length < validatedData.qty) {
                transactionError = `Stock is low, only ${availableStock.length} products available`;
                tx.rollback(); // undo all previous database's query
                return;
            }
            ...
            await tx
                ...
                .where(
                    inArray( // set orderid in all row where [inventories.id == availableStock.map((stock) => stock.id)]
                        inventories.id,
    ...
    const paymentData = {
        ...
        url_callback: `${process.env.APP_BASE_URL}/api/payment/callback`, // App_base_url should not be localhost:3000 - use https://ngrok.com/ - it will create a public link of your localhost:3000 protocol. | Cryptomous webhook callback does not work in localhost so ngrock will help in dev process.
    };
