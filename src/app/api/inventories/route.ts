...
export async function GET() {
    try {
        const allInventories = await db
            ...
            .from(inventories)
            .leftJoin(warehouses, eq(inventories.warehouseId, warehouses.id)) // get data of warehouse from ID where (inventories.warehouseId === warehouses.id) 
            .leftJoin(products, eq(inventories.productId, products.id)) // get data of inventory from ID where (inventories.productId === products.id)
